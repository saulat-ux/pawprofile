...
import { RESET_BREED,editBreed ,getBreeds } from '../../redux/features/breed/breedSlice';
import { Link, useNavigate } from 'react-router-dom';




const EditForm = ({trigger,dogId ,onClose}) => {
  ...
  const handleSubmit =  async () => {
    ...
    await dispatch(editBreed({ id:dogId, name, imageURL, description }))
  ...
  return trigger ? (
      ...
      <div className="max-w-md mx-auto bg-white rounded p-6 shadow-md">
        <h2 className="text-2xl mb-4 text-black">Edit Information </h2>
        <form onSubmit={handleSubmit}>
          ...
          <button className='bg-black text-white py-2 mr px-4 rounded-md' onClick={() => {onClose()}}>Close</button>
